...
答案是不够的，因为这里有个“或”的条件。但有向无环图可以。（当然用WFCounterTask也是可以的）
...
对图和节点，可以梳理出以下功能：
- 节点任务WFGraphNode：在前序节点都完成时，调起它接管的task；在task结束时后，告知后序节点；
- 图任务WFGraphTask：整个图被调起时，并发调起所有节点，让他们按依赖运行；整个图结束之后，调用用户回调函数；

那么很显然，可以分别用WFCounterTask和ParallelWork来实现。是的，我们已经快要和作者一起设计出DAG的模块了！

- WFGraphNode派生自WFCounterTask，数够N个数，当前任务流就往下走；
- WFGraphTask派生自WFGenericTask，内部发起一个ParallelWork；

有些小伙伴可能对WFGenericTask比较陌生，但其实，它是Workflow多种基础任务中的万金油。据不完全统计（作者手动grep._.），WFGenericTask是最多子类的一个基础任务。

### 3.1 还是先看图：WFGraphTask
...
WFGraphTask  WFGenericTask 

WFGraphNode WFCounterTask 计数器

4、最后引用。。。

数据传递、低代码开发等，都是可以发挥的空间！

在 Ole-Johan Dahl、Dijkstra 和 C.A.R Hoare 的《Structured Programming》一书中提到，应该使用抽象的控制结构来组织代码。该书针对结构化并发其实还提出了许多约束，是的，不是新功能，而是约束，其突破性的思想在于，正是需要这些约束，才能让并发的程序得到清晰、明确的运行模式。
